...
export default function CadastroImoveis() {
    ...
    async function buscarImoveis() {
      try {
        ...
        const dadosNormalizados = data.map((item) => ({
          ...item,
          imagens: Array.isArray(item.imagens) ? item.imagens.map(normalizarImagem) : [],
  ...
  async function handleCapa(e) {
    ...
    try {
      ...
      const novas = form.imagens.length === 0 
        ? urls 
  ...
  const customSelectStyles = {
    control: (base, state) => ({
      ...
      "&:hover": {
        borderColor: state.isFocused ? "#c1473d" : "#ccc"
      },
      cursor: 'pointer',
    }),
    valueContainer: (base) => ({
    ...
    option: (base, state) => ({
      ...
      padding: "10px 12px",
      "&:hover": {
        backgroundColor: "rgba(193, 71, 61, 0.1)"
      },
    }),
  ...
  function toggleEstrutura(item) {
    if (form.estrutura.includes(item)) {
      setForm({ 
        ...form, 
        estrutura: form.estrutura.filter(i => i !== item) 
      });
    } else {
      setForm({ 
        ...form, 
        estrutura: [...form.estrutura, item] 
      });
    }
  }
    ...
    <div className="container">
      ...
      <p className="subtitulo">
        {editandoId 
          ...
          : "Preencha os dados abaixo para adicionar um novo imóvel ao catálogo."
        }
      </p>
            ...
            {form.imagens.length === 0 ? (
              ...
              <div className="preview-capa-wrapper">
                <img 
                  src={form.imagens[0]} 
                  alt="Capa do imóvel" 
                  className="preview-capa-img" 
                />
                <div className="preview-capa-actions">
                  <label htmlFor="fileInputCapa" className="preview-capa-btn">
                    Alterar capa
                  </label>
                  <input
                    id="fileInputCapa"
                ...
                {erros.imagens && (
                  <span className="campo-erro">Adicione pelo menos a foto de capa.</span>
                )}
              </div>
            )}
