...
function Catalogo() {
    ...
    fetch("https://zimimoveis-production.up.railway.app/imoveis")
      ...
      .then(data => {
        const tratados = data.map((item) => ({
          ...item,
          imagens: typeof item.imagens === "string" 
  ...
  function filtrarImoveis(filtro) {
    ...
    if (filtro.tipo?.value) {
      resultado = resultado.filter(
        (i) =>
          i.tipo?.toLowerCase().trim() === filtro.tipo.value.toLowerCase().trim()
      );
    }
  ...
  // ////////////////////////////////////////////////////
  const base = listaFiltrada && listaFiltrada !== "VAZIO" 
    ? listaFiltrada 
    : imoveis;

  const baseLista = listaFiltrada && listaFiltrada !== "VAZIO" 
    ? listaFiltrada 
    : imoveis;
  ...
  const listaFinal = listaFiltrada === "VAZIO" ? [] : listaComBusca;

  return (
    <div className="catalogo">
      <Pesquisa 
        setBusca={setBusca} 
        abrirFiltro={() => setPopupAberto(true)} 
      />
        ...
        <div className="cards-grid">
          {listaFinal.map((item) => (
            ...
            >
              <Card 
                {...item} 
                imagens={item.imagens || [item.img_capa]} 
              />
            </Link>
          ))}
        </div>
